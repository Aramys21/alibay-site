...
app.get('/addCreditCard', async (req, res) => {
  try {
    console.log(req.session.username);
    res.render('ajouterunecarte.ejs', {username : req.session.username,loggedIn : req.session.loggedin});
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal server error');
  }
...
app.get('/vetements', async (req, res) => {
  try {
    res.render('vetements.ejs', {username : req.session.username,loggedIn : req.session.loggedin});
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal server error');
  }
...
app.get('/product-page', async (req, res) => {
  try {
    res.render('product-page.ejs', {username : req.session.username,loggedIn : req.session.loggedin});
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal server error');
  }
...
app.get('/loginPage', async (req, res) => {
  try {
    res.render('login.ejs', {username : req.session.username,loggedIn : req.session.loggedin});
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal server error');
  }
...
app.get('/registerPage', async (req, res) => {
  try {
    res.render('register.ejs', {username : req.session.username,loggedIn : req.session.loggedin});
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal server error');
  }
